    ...
    <div>
        ...
        <sui-form v-if="state.needsRecordType && !validationError">
            <sui-form-field :required="true" :error="state.showErrors && !state.recordType">
                ...
                <Dropdown defaultText="Select Record Type"
                          v-model="state.recordTypeSaved" :options="state.recordTypes" text-field="name"
                          value-field="recordTypeId"></Dropdown>
            </sui-form-field>
        </sui-form>
        ...
        <sui-form v-if="!state.needsRecordType && !validationError && state.layout">
            <fragment v-for="(section, key) in state.layout.editLayoutSections">
                <h4 class="ui header">{{ section.heading }}</h4>
                <div class="two fields" v-for="(row, key) in section.layoutRows">
                    <fragment v-for="(item, key) in row.layoutItems" :key="key">
                        <ObjectFormField v-for="(comp, key) in item.layoutComponents"
                                         :key="key"
                                         v-if="!comp.exclude"
                                         :field="comp.field"
                                         :showError="state.showErrors" :label="item.label"
...
<script>
    ...
    export default {
        ...
        setup: function(props) {
            ...
            watch(() => [props.objectType, state.recordTypeId, state.schema],
                async () => {
                    if (!props.objectType) {
                        return
                    }
                    ...
                    if (includeFields.length) {
                        ...
                        try {
                            ...
                            console.warn('layout support for this env not found. Cache table needs not only schema but layout describes as well')
                        }
            ...
            const validationError = computed(() => {
                ...
                if (props.id && props.id.startsWith('PIT_') && !table.externalField) {
                    ...
                    validationErrorDescription.value = 'This Object does not have an external ID field in the config.json. Please provide one or sync first before editing.'
                    return true
                }
                return false
            })
            ...
            watch(() => [props.id, state.fields], async () => {
